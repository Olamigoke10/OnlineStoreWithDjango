...
class Order(models.Model):
    ...
    def update_status(self):
        ...
        if self.status == 'Pending':
            if time_since_creation >= timezone.timedelta(minutes=1):#  Change to 'Confirmed' after 1 minute for testing
                self.status = 'Confirmed'
                self.save()
        elif self.status == 'Confirmed':
            if time_since_creation >= timezone.timedelta(minutes=2):  # Change to 'Delivered' after 2 minutes for testing
